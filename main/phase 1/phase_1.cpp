...
// SCANNING DATA FILE
void readfile (int a[maxm][maxn], int &prec, int &plan_time) {
	FILE *f;
...
void writefile (int a[maxm][maxn], int prec, int plan_time) {
	FILE *f;
    double sum=0;
...
int main () {
	int a[maxm][maxn];
    int plan_time, job_scale;
    int prec;
    readfile(a, prec, plan_time);
    writefile(a, prec, plan_time);
	return 0;
} 